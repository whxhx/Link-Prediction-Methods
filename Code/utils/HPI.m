function [ thisauc ] = HPI( train, test, nodedegree )
%% 计算HPI指标并返回AUC值
% changed by geyao
% %     sim = train * train;     
%     sim = trainsquare;
...
%     nodedegree = sum(train, 2)';
    ...
    [i,j] = find(test);
    for k = 1 : length(i)
        test_data(1,k) =  sum(train(i(k),:) .* train(j(k),:)) / min(nodedegree(1, i(k)) , nodedegree(1, j(k)));
    end
    ...
    for i = limiti
        ...
        for j = limitj
            ...
            if ~train(i,j) && ~test(i,j) && i~= j
                non_data(1,k) = sum(train(i,:) .* train(j,:)) / min(nodedegree(1, i) , nodedegree(1, j));
                k = k + 1;
            end
        end
    end
